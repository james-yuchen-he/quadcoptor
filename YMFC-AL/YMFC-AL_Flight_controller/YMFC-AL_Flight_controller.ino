...
//  }
  ...
  angle_pitch_acc -= -3.3;                                                   //Accelerometer calibration value for pitch.
  ...
  if (start == 2){                                                          //The motors are started.
    ...
    esc_1_cmd = throttle - pid_output_pitch + pid_output_roll - pid_output_yaw; //Calculate the pulse for esc 1 (front-right - CCW)
    esc_2_cmd = throttle + pid_output_pitch + pid_output_roll + pid_output_yaw; //Calculate the pulse for esc 2 (rear-right - CW)
    esc_3_cmd = throttle + pid_output_pitch - pid_output_roll - pid_output_yaw; //Calculate the pulse for esc 3 (rear-left - CCW)
    esc_4_cmd = throttle - pid_output_pitch - pid_output_roll + pid_output_yaw; //Calculate the pulse for esc 4 (front-left - CW)

    // Turning off voltage compensation for now.
    // if (battery_voltage < 1240 && battery_voltage > 800){                   //Is the battery connected?
    //   esc_1_cmd += esc_1_cmd * ((1240 - battery_voltage)/(float)3500);              //Compensate the esc-1 pulse for voltage drop.
    //   esc_2_cmd += esc_2_cmd * ((1240 - battery_voltage)/(float)3500);              //Compensate the esc-2 pulse for voltage drop.
    //   esc_3_cmd += esc_3_cmd * ((1240 - battery_voltage)/(float)3500);              //Compensate the esc-3 pulse for voltage drop.
    //   esc_4_cmd += esc_4_cmd * ((1240 - battery_voltage)/(float)3500);              //Compensate the esc-4 pulse for voltage drop.
    // }

    if (esc_1_cmd < 1100) esc_1_cmd = 1100;                                         //Keep the motors running.
    if (esc_2_cmd < 1100) esc_2_cmd = 1100;                                         //Keep the motors running.
    if (esc_3_cmd < 1100) esc_3_cmd = 1100;                                         //Keep the motors running.
    if (esc_4_cmd < 1100) esc_4_cmd = 1100;                                         //Keep the motors running.

    if(esc_1_cmd > 2000)esc_1_cmd = 2000;                                           //Limit the esc-1 pulse to 2000us.
    if(esc_2_cmd > 2000)esc_2_cmd = 2000;                                           //Limit the esc-2 pulse to 2000us.
    if(esc_3_cmd > 2000)esc_3_cmd = 2000;                                           //Limit the esc-3 pulse to 2000us.
    if(esc_4_cmd > 2000)esc_4_cmd = 2000;                                           //Limit the esc-4 pulse to 2000us.
  }

  else{
    esc_1_cmd = 1000;                                                           //If start is not 2 keep a 1000us pulse for ess-1.
    esc_2_cmd = 1000;                                                           //If start is not 2 keep a 1000us pulse for ess-2.
    esc_3_cmd = 1000;                                                           //If start is not 2 keep a 1000us pulse for ess-3.
    esc_4_cmd = 1000;                                                           //If start is not 2 keep a 1000us pulse for ess-4.
  }
